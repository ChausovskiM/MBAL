...
def main():
    ...
    #
    #=== расчет плотности
    rho = Density_calc('корреляция', P_Mpa, T_C_plast, Z_calculate)
    rho_std = Density_calc('корреляция', 0.101325, 20, 1)
    #===
    #
    ...
    df['Density'] = df.apply(lambda row: Density_calc('корреляция',row['P_MPA'],T_C_plast,row['ZBB']), axis =1)

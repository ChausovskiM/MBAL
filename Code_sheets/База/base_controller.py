...
# инпуты листа PZ
    ...
    oiz_gas = pz_input['nbz_gas'] - pz_input['Cum_gas_under_pred'] #оизы на дату начала прогноза
    # Инпуты листа БАЗА
    ...
    with open(r"code_sheets\Productivity\productivity_outputs.json", 'r', encoding='utf-8') as f:
        product_outputs = json.load(f)
    product_outputs_df = pd.DataFrame(product_outputs["results_table"])
    #
    #
    ...
    df_tabl.loc[0,'oiz_gas'], df_tabl.loc[0,'oiz_cond'] = oiz_gas - df_tabl['Qgas_all'].iloc[0], 0
